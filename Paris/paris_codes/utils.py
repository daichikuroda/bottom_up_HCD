...
class hsbm:
    def __init__(self, numbers, intensities):
        ...
        size = np.prod(numbers)  # the size of the adjancy matrix = number of nodes
        # creating poaison lambda matrix = expected number matrix
        self._matrix = self._mus[0] * np.ones((size, size))
        for l in range(1, len(self._mus)):
            for b in range(np.prod(numbers[:l])):
                size_l = size // np.prod(numbers[:l])  # "//" = quotient
                self._matrix[
...
def resolution_analysis(G, resolutions):
    resolution_range = np.logspace(
        np.log10(resolutions[-1]), np.log10(resolutions[0]), num=100
    )
    ...
    for r in resolutions:
        plt.axvline(x=r, color="k", alpha=0.2)
    plt.show()
